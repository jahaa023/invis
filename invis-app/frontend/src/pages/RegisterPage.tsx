...
export default function RegisterPage() {
    ...
    const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (response.ok) {
                ...
                if (
                    response.status == 400 &&
                    res_json.error.code == "VALIDATOR_ERROR"
                ) {
    ...
    return (
        <div className="w-screen h-screen flex justify-center items-center">
            <form
                className="w-100 h-fit p-8 bg-bg-black-lighter border-black-lighter-border border-[1px] rounded-2xl flex flex-col items-center gap-3 text-center max-h-[80vh] overflow-y-scroll"
                onSubmit={handleSubmit}
            >
                <img
                    src="/favicon.svg"
                    alt="Invis Planet Logo"
                ...
                {isFocused && !usernameValidationError && (
                    <div className="p-2 w-[calc(100%-0.5rem)] border-1 border-text-light rounded-md font-light text-left text-nowrap text-xs">
						<p className={usernameValidation.chars ? "text-warning-red" : "text-positive-green"}>
                            Must be between 5-20 characters.
                        </p>
						<p className={usernameValidation.whitespace ? "text-warning-red" : "text-positive-green"}>
                            Must not contains whitespace like spaces.
                        </p>
						<p className={!usernameValidation.ascii ? "text-warning-red" : "text-positive-green"}>
                            Must only contain ASCII characters
                        </p>
						<p className={usernameValidation.taken ? "text-warning-red" : "text-positive-green"}>
                            Must not be taken
                        </p>
                    </div>
                )}
                {usernameValidationError && isFocused && (
                    <div className="p-2 w-[calc(100%-0.5rem)] border-1 border-text-light rounded-md font-light text-left text-nowrap text-xs">
                ...
                <div className="flex items-center w-[calc(100%-0.5rem)] h-10 min-h-10 border-1 border-text-light rounded-md font-light relative">
                    <input
                    ...
                    >
                        {visibilty ? (
                            <MdVisibility className="w-[60%] h-[60%]" />
                        ) : (
                            <MdVisibilityOff className="w-[60%] h-[60%]" />
                        )}
                    </button>
                </div>
                <button
