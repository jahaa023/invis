                ...
                <div className="w-full bg-bg-header-button p-3 rounded-md">
                    ...
                    <div className="w-full h-72 overflow-x-hidden overflow-y-scroll p-2">
                        ...
                        {!searchError &&
                            !searchNotFound &&
                            searchResult &&
                            searchResult.map((row: searchUser) => (
                                <div key={row.user_id}>
                                    <div className="w-full h-[1px] bg-black-lighter-border my-2"></div>
                                    <div className="flex items-center gap-3 relative hover:bg-bg-black-lighter p-2 rounded-sm">
                                        <img
                                            className="w-10 h-10 rounded-full border-bg-header-button border-2"
                    ...
                    outgoingRequests.length === 0 ? (
                        <div className="w-full h-full sm:flex justify-center items-center">
                            <div className="sm:text-center text-left">
                                <h1 className="font-bold text-4xl opacity-50">
                                    No requests yet...
                                </h1>
                                <p className="opacity-50">
                                    Tell your friend to send a friend request!
                                </p>
                            </div>
                        </div>
                    ) : (
                        <div className="w-full bg-bg-header-button p-3 rounded-md">
                            <div className="mb-5">
                                <p className="font-medium opacity-50 font-stretch-expanded">
                                ...
                                {incomingRequests.length > 0 ? (
                                    <>
                                        {incomingRequests.map(
                                            (row: friendRequest) => {
                                                return (
                                                    <div className="w-full p-2 flex items-center bg-background-black rounded-md gap-1.5 relative">
                                                        <img
                                                            className="w-8 h-8 rounded-full border-black-lighter-border border-2"
                                                            src={
                                                                row.profile_picture_url
                                                            }
                                                        />
                                                        <p className="font-medium max-w-[35%] overflow-hidden overflow-ellipsis">
                                                            {row.username}
                                                        </p>
                                            ...
                                            (row: friendRequest) => {
                                                return (
                                                    <div className="w-full p-2 flex items-center bg-background-black rounded-md gap-1.5 relative">
                                                        ...
                                                        />
                                                        <p className="font-medium max-w-[50%] overflow-hidden overflow-ellipsis">
                                                            {row.username}
                                                        </p>
                                                        <button
